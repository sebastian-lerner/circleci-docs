...
NOTE: If your services are externalized then you may run `./migrate.sh --skip-databases` to skip Postgres and Mongo. In the event Postgres, Mongo, and Vault are all externalized, you simply need to `scp` (secured copy) your keys from `/data/circle/circleci-encryption-keys` on the v2.19.x services machine.
