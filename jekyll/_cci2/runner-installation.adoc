...
```bash
mkdir -p /opt/circleci/policy

sepolicy generate --path /opt/circleci/policy --init /opt/circleci/circleci-launch-agent
```

Copy the following type enforcing file to `/opt/circleci/policy/circleci_launch_agent.te`, owned by `root`:
...
Finally, install the policy using the setup script generated in a previous step:
